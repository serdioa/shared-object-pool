...
public class ConcurrentSharedObjectPool<K, S extends SharedObject, P extends PooledObject> implements SharedObjectPool<K, S> {
    ...
    private Entry getEntry(K key) throws InvalidKeyException {
        return this.entries.computeIfAbsent(key, this::createEntry);
    }
    ...
    private class Entry {
        ...
        synchronized S createSharedObject() {
            ensureActive();
            S sharedObject = ConcurrentSharedObjectPool.this.createSharedObject(this.pooledObject, this::disposeSharedObject);

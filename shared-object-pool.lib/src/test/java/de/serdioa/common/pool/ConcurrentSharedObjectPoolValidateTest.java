...
public class ConcurrentSharedObjectPoolValidateTest {

    public static void main(String[] args) throws Exception {
        ConcurrentSharedObjectPoolValidateTest test = new ConcurrentSharedObjectPoolValidateTest();
        test.runRepeated(4, 4, 1, 2000, 100);
    }


    private ConcurrentSharedObjectPool<String, SharedCounter, PooledCounter> buildImmediatePool() {
    ...
    private ConcurrentSharedObjectPool<String, SharedCounter, PooledCounter> buildDelayedPool(long disposeDelayMillis) {
        PooledObjectFactory<String, PooledCounter> pof = new PooledCounterFactory();
        SharedObjectFactory<PooledCounter, SharedCounter> sof = SynchronizedSharedObject.factory(SharedCounter.class);
    ...
    private int run(int keys, int workers, int disposeDelayMillis, long durationMillis) throws InterruptedException {
        ConcurrentSharedObjectPool<String, SharedCounter, PooledCounter> pool;
        ...
        if (disposeDelayMillis > 0) {
            final long sleepTime = disposeDelayMillis + 500;
            System.out.println("Waiting " + sleepTime + " ms for async tasks to dispose of pooled objects");
            Thread.sleep(sleepTime);
            System.out.println("Finished waiting for async tasks");
        }
